...
def get_extended_group_lengths(board, player=1):
    """
    For each column in the board, simulate dropping a piece for 'player'.
    If the move extends an existing group (i.e. the connected group length > 1),
    record the new group length (the maximum over four directions).
    
    Returns:
        A list of the new group lengths for each column where the move extends a group.
    """
    rows, cols = board.shape
    extended_lengths = []
    
    # For each column, determine the lowest empty row (simulate gravity)
    for col in range(cols):
        empty_rows = np.where(board[:, col] == 0)[0]
        if empty_rows.size == 0:
            # Column is full, skip it.
            continue
        row = empty_rows[-1]  # bottom-most empty row in this column
        
        # Place the piece temporarily.
        board[row, col] = player
        
        max_length = 1  # at least the newly placed piece
        # Check the four directions: horizontal, vertical, and the two diagonals.
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        for dr, dc in directions:
            count = 1  # Count includes the newly placed piece
            
            # Explore both opposite directions.
            for sign in [-1, 1]:
                r, c = row, col
                while True:
                    r_next = r + sign * dr
                    c_next = c + sign * dc
                    if 0 <= r_next < rows and 0 <= c_next < cols and board[r_next, c_next] == player:
                        count += 1
                        r, c = r_next, c_next
                    else:
                        break
            # Save the maximum group length found among the directions.
            max_length = max(max_length, count)
        
        # Remove the temporary piece.
        board[row, col] = 0
        
        # Only record the column if the new piece would extend an existing group (i.e. group length > 1)
        if max_length > 1:
            extended_lengths.append(max_length)
    
    return extended_lengths


class ConnectFourGymV13(ConnectFourGymV5):
    def __init__(self, agent2="random"):
        super().__init__(agent2)
        # Tuple corresponding to the min and max possible rewards
        self.reward_range = (-10, 5)
    ...
    def change_reward(self, old_reward, done):
        ...
        if old_reward == 1:  # The agent won the game
            ...
            groups = get_extended_group_lengths(board, mark)
            val = sum(0.01 * x for x in groups)
            return val