...
value_dict_origin = model.learner.policy.to("cpu").state_dict()

policy_keys = state_dict_origin.keys()
value_keys = value_dict_origin.keys()
...
for key in value_keys:
    if key.startswith("value"):
        value_net_dict[key.replace("_net", "")] = value_dict_origin[key]
    elif "value_net" in key:
        els = key.split(".")
        if len(els) < 3:
            continue
        i = int(els[2])
        prefix = "value" if "value_net" in key else "shared"
        policy_index = i // 2 + 1
        value_net_dict[f"{prefix}{policy_index}.{els[-1]}"] = value_dict_origin[key]


p = path.join(path.dirname(__file__))

env = Environment(loader=FileSystemLoader(p))
template = env.get_template("submission_template_2.jinja.py")


output_submission = template.render(state_dict=state_dict, value_net_dict=value_net_dict)

with open("submission_2.py", "w") as fh:
