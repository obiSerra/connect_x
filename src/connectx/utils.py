...
Results = namedtuple("Results", "agent1_wins agent2_wins agent1_inv agent2_inv")


def get_win_percentages_first(agent1, agent2, n_rounds=100):
...
def get_win_percentages_last(agent1, agent2, n_rounds=100):
    # Use default Connect Four setup
    config = {"rows": 6, "columns": 7, "inarow": 4}
    # Agent 1 goes first (roughly) half the time
    outcomes = evaluate("connectx", [agent2, agent1], config, [], n_rounds)
...
def get_agentV2(model):
    ...
    def agent(obs, config):
        try:
            ...
            action, _ = model.predict({"board": obs_reshaped, "mark": obs["mark"]})
            is_valid = any(board_2d[:, int(action)] == 0)
...
def get_agentV3(model):
    print("Getting agent v3")
...
def update_model_data(model_name, version, results_random, results_look, results_negamax=None):
    with open(f"data/progress_{model_name}.csv", "a") as file:
        lines = [
            ...
            f"{results_negamax.agent1_wins},",
            f"{results_negamax.agent1_inv},",
            f"{datetime.now()}",
            "\n",
        ]
        # Writing to file
